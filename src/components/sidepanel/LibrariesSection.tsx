...
const TOOLS: LibraryTool[] = [
  ...
  },
  {
    ...
    // tshark is the CLI companion, is faster, and doesn't open a GUI window.
    // The absolute paths are tried directly (no shell), bypassing PATH entirely.
    checkCmd: 'tshark --version',
    checkCmdFallbacks: [
  ...
  // LLM
  {
    ...
    description: 'Run LLMs locally — used by NexShell AI chat & autocomplete',
  ...
  },
  {
    ...
    description: 'Desktop app for running local LLMs with built-in server',
    checkCmd: 'lms version',
    installCmds: {
      win: 'winget install ElementLabs.LMStudio',
      mac: 'brew install --cask lm-studio',
    },
...
/** Run tool checks CONCURRENCY at a time, updating state as each resolves so
 * the UI fills in progressively rather than all-at-once after a long pause. */
...
export default function LibrariesSection() {
  const { theme, libraryFilter } = useStore()
  const ui = theme.ui
  ...
  // Consume libraryFilter from store (e.g. navigating from AI Settings)
  useEffect(() => {
    if (libraryFilter) {
      setActiveCategory(libraryFilter)
      setState({ libraryFilter: null })
    }
  }, [libraryFilter])
  ...
  useEffect(() => {
    async function checkAll() {
      ...
      await runChecks(TOOLS, (id, installed, version) => {
        setToolStates(prev => ({ ...prev, [id]: { status: installed ? 'installed' : 'missing', version } }))
      })
    }
